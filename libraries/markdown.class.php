...
class Markdown extends Base{
	...
	public function set_image_prefix($prefix){
	...
	protected function _doImages_reference_callback($matches) {
		...
		if (isset($this->urls[$link_id])) {
			...
			if(strpos(trim($url), 'http://') === 0){
				...
				if($count >= 2 && $url_parts[$count - 2] == 'images'){
					$images_name = $url_parts[$count - 1];
					$url = $this->image_prefix . $images_name;
				}
			}
			...
			$result = "<img class=\"img-thumbnail img-responsive\" src=\"$url\" alt=\"$alt_text\"";
			if (isset($this->titles[$link_id])) {
				$title = $this->titles[$link_id];
				$title = $this->encodeAttribute($title);
				$result .=  " title=\"$title\"";
	...
	protected function doBlockQuotes($text) {
		$text = preg_replace_callback('/
			  (								# Wrap whole match in $1
				(?>
				  ^[ ]*&gt;[ ]?	# xpp change ^[ ]*>[ ]? to ^[ ]*&gt;[ ]?		 ">" at the start of a line
	...
	protected function _doBlockQuotes_callback($matches) {
		...
		#$bq = preg_replace('/^[ ]*>[ ]?|^[ ]+$/m', '', $bq); xpp change
...
//				}
	...
	protected function doAutoLinks($text) {
		/*$text = preg_replace_callback('{<((https?|ftp|dict):[^\'">\s]+)>}i',
				...
				xpp: because we transfer html entity already, change schema here
				*/
